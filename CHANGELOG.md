...
- `teleport ship` resolves the local binary from `profile.bin_file` automatically when configured, skipping the picker on every run. Remote filename priority: `--name` flag > `profile.remote_name` > local basename. Ship now runs a bubbletea animated TUI (spinner + progress bar) identical in style to `teleport sync`, showing each step (uploading → chmod +x → moving) with live spinner and 1/3 → 2/3 → 3/3 progress, plus a final summary line with destination path, OS, and elapsed time.
