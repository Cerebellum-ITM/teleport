...
- The beam send/progress view now prefixes each file with its commit's colored cube (`󰆧`), matching the file picker's per-commit coloring, so you can tell at a glance which commit each uploaded file came from. Colors are computed from the same change set the picker used, so they stay consistent between the two views.
