...
- `teleport ship` resolves the local binary from `profile.bin_file` automatically when configured, skipping the picker on every run. Remote filename priority: `--name` flag > `profile.remote_name` > local basename. Ship prints all steps upfront (ANSI cursor movement updates each line in place — yellow spinner while active, green ✓ on success); the upload line shows live byte progress (`3.2 MB / 8.1 MB  39%`). When the remote name differs from the local filename, an explicit `renaming  original → remote` step appears between upload and chmod. Upload speed improved: SFTP switched to `MaxPacketUnchecked(1<<22)` + `UseConcurrentWrites` + `sftp.File.ReadFrom` (reads in 4 MiB chunks on the fast concurrent path instead of 32 KB `io.Copy` writes).
