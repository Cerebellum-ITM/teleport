...
- `teleport ship` resolves the local binary from `profile.bin_file` automatically when configured, skipping the picker on every run. Remote filename priority: `--name` flag > `profile.remote_name` > local basename. Ship prints log-style spinner lines (yellow while active → green on success) for each step; the upload line shows live byte progress (`3.2 MB / 8.1 MB  39%`) updated in place. Upload speed improved significantly: SFTP packet size raised to 1 MiB and concurrent writes enabled (`sftp.MaxPacketChecked(1<<20)` + `sftp.UseConcurrentWrites`), replacing the 32 KiB default that caused ~47 s transfers for small binaries.
